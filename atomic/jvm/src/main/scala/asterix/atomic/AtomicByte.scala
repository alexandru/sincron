...
final class AtomicByte private (ref: JavaAtomicInteger)
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForCompareAndSet(expect: Byte, update: Byte, waitUntil: Long): Unit =
    if (!compareAndSet(expect, update)) {
      interruptedCheck()
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForValue(expect: Byte, waitUntil: Long): Unit =
    if (get != expect) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForValue(expect, waitUntil)
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForCondition(waitUntil: Long, p: Byte => Boolean): Unit =
    if (!p(get)) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForCondition(waitUntil, p)
