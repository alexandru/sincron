...
final class AtomicLong private (ref: JavaAtomicLong) 
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForCompareAndSet(expect: Long, update: Long, waitUntil: Long): Unit =
    if (!compareAndSet(expect, update)) {
      interruptedCheck()
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForValue(expect: Long, waitUntil: Long): Unit =
    if (get != expect) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForValue(expect, waitUntil)
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForCondition(waitUntil: Long, p: Long => Boolean): Unit =
    if (!p(get)) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForCondition(waitUntil, p)
