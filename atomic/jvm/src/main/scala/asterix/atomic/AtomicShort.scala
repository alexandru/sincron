...
final class AtomicShort private (ref: JavaAtomicInteger)
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForCompareAndSet(expect: Short, update: Short, waitUntil: Long): Unit =
    if (!compareAndSet(expect, update)) {
      interruptedCheck()
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForValue(expect: Short, waitUntil: Long): Unit =
    if (get != expect) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForValue(expect, waitUntil)
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[asterix] def waitForCondition(waitUntil: Long, p: Short => Boolean): Unit =
    if (!p(get)) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForCondition(waitUntil, p)
