...
final class AtomicBoolean private (ref: JavaAtomicBoolean) extends BlockableAtomic[Boolean] {
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[sincron] def waitForCompareAndSet(expect: Boolean, update: Boolean, waitUntil: Long): Unit =
    if (!compareAndSet(expect, update)) {
      interruptedCheck()
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[sincron] def waitForValue(expect: Boolean, waitUntil: Long): Unit =
    if (get != expect) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForValue(expect, waitUntil)
  ...
  @throws(classOf[InterruptedException])
  @throws(classOf[TimeoutException])
  private[sincron] def waitForCondition(waitUntil: Long, p: Boolean => Boolean): Unit =
    if (!p(get)) {
      interruptedCheck()
      timeoutCheck(waitUntil)
      waitForCondition(waitUntil, p)
